...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--no-headless", action="store_true",
                    help="show the Selenium browser window")
    ...
    if args.source in ("all", "bonds"):
        if not storage.bonds_db_exists():
            log.error("bonds DB missing at %s — run scripts/migrate_bonds_db.py first",
    ...
    if args.source in ("all", "equities"):
        log.info(
            "equity data refresh is delegated to global_data_storage. "
            "Run its ingest CLI from %s",
