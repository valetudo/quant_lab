...
def attribute_by_instrument(trades: Iterable) -> pd.DataFrame:
    ...
    if df.empty:
        return df
    return (df.groupby("instrument", as_index=False)
...
def attribute_by_metadata_field(trades: Iterable, field: str) -> pd.DataFrame:
    ...
    if df.empty:
        return df
    return (df.groupby("bucket", as_index=False)
              .agg(total_pnl=("pnl", "sum"), n=("pnl", "count"),
                   avg_pnl=("pnl", "mean"), avg_duration=("duration", "mean"))
              .sort_values("total_pnl", ascending=False))
