...
from quant_lab.portfolio.master_allocator import EqualWeightAllocator, MasterAllocator
...
class PortfolioAggregator:
    ...
    def run(self, start: Optional[date_type] = None, end: Optional[date_type] = None) -> PortfolioResult:
        weights = self.allocator.compute_weights(
            pd.Timestamp(start) if start else pd.Timestamp.today(),
            strategy_states={}, market_data=None,
        )
        ...
        for strat in self.strategies:
            ...
            metrics = compute_metrics(eq, res.trades, cap,
                                      open_count=res.open_count,
                                      exposure=res.exposure)
            result.strategy_results[sid] = StrategyRunResult(
                strategy_id=sid, equity=res.equity, trades=res.trades,
                metrics=metrics, capital_alloc=cap,
